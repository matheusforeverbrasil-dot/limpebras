<?php
// =================================================================
// SISTEMA_RASTREABILIDADE.PHP - BUSCA, RESULTADOS E AUTOCOMPLETAR (COMPLETO)
// Este arquivo depende da existência dos scripts: gerar_ficha_pdf.php e exportar_csv.php
// =================================================================
...
if ($conexao_status && isset($_GET['action']) && $_GET['action'] == 'autocomplete') {
    ...
    if (isset($_GET['termo']) && !empty(trim($_GET['termo']))) {
        ...
        if ($stmt === false) {
             echo json_encode(['error' => 'Erro SQL Sugestão: ' . $conn->error]);
             $conn->close();
             exit;
        }
...
// -----------------------------------------------------------------
// 🚨 3. LÓGICA DE PROCESSAMENTO (BUSCA PRINCIPAL DE FICHAS)
// -----------------------------------------------------------------
if ($conexao_status && $_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET['termo'])) {
    ...
    if (!empty($termo_pesquisa)) {
        ...
        // CORREÇÃO SQL: Usando JOIN com a tabela 'setores' para obter o nome do setor
        $sql = "
            SELECT 
                e.numero_ordem, 
                e.data_entrega, 
        ...
        if ($stmt === false) {
            // Captura o erro de sintaxe SQL da query principal
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            input.addEventListener('input', function() {
                ...
                timeout = setTimeout(function() {
                    // Chama o próprio script com o parâmetro de ação AJAX

...
// =================================================================
// 4. PREPARAÇÃO DA CONSULTA PRINCIPAL (Com Filtros)
// =================================================================
...
if ($stmt = $conn->prepare($sql)) {
    ...
    // Adiciona os mesmos WHEREs para a contagem
    $count_params = [];
    $count_types = '';

    if ($filtro_setor > 0) { $sql_count .= " AND setor_id = ?"; $count_params[] = $filtro_setor; $count_types .= 'i'; }
    if ($filtro_funcao > 0) { $sql_count .= " AND funcao_id = ?"; $count_params[] = $filtro_funcao; $count_types .= 'i'; }
    if ($filtro_status !== -1) { $sql_count .= " AND status = ?"; $count_params[] = $filtro_status; $count_types .= 'i'; }
    
    $stmt_count = $conn->prepare($sql_count);
    
    // === CORREÇÃO CRÍTICA DO BIND_PARAM DINÂMICO (CONTADOR) ===
    if (!empty($count_params)) {
        $bind_params_count = array_merge([$count_types], $count_params);
        $refs_count = [];
    ...
    // === CORREÇÃO CRÍTICA DO BIND_PARAM DINÂMICO (CONSULTA PRINCIPAL) ===
    if (!empty($params)) {
        // Usa os mesmos $params e $types, que são as variáveis usadas na consulta principal $sql
        $bind_params_main = array_merge([$types], $params);
        $refs_main = [];
        foreach ($bind_params_main as $key => $value) {
            $refs_main[$key] = &$bind_params_main[$key];
        }
        call_user_func_array([$stmt, 'bind_param'], $refs_main);
    }
    // ===================================================================
...
// 5. INCLUSÃO DO CABEÇALHO E INÍCIO DO HTML
require_once 'header.php'; 
// O header.php deve ter: <!DOCTYPE html>, <html>, <head>, <body>
